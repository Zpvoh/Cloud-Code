...
export default {
  ...
  activate(state) {
    ...
    bucket.listObjects( (err, data) => {
      ...
      this.cloudCodeView.getElement().appendChild(this.cloudCodeView.createButton('Download',['btn','btn-lg'],() => {
        var fs=require('fs');
        ...
        function(err, data) {
          ...
          console.log(this.cloudCodeView.getSelectedItemPath()[0]);
          fs.writeFile(filePath, data.body);
          atom.workspace.open(filePath);
        });
      }));

      this.cloudCodeView.getElement().appendChild(this.cloudCodeView.createButton('Refresh',['btn','btn-lg'],() => {
        var configArr=this.cloudCodeView.config;
        var config = new Config(configArr[2].getAttribute('value'), configArr[3].getAttribute('value'));
        var bucket = new QingStor(config).Bucket(configArr[0].getAttribute('value'), configArr[1].getAttribute('value'));
        bucket.listObjects((err, data)=>{
          this.data=data;
          this.cloudCodeView.files=this.toJSON(this.toKeyArray(this.data.keys));
          console.log(this.data.keys);
      ...
      this.cloudCodeView.getElement().appendChild(this.cloudCodeView.createButton('Upload',['btn','btn-lg'],() => {
        var fs=require('fs');
        var editor=atom.workspace.getActiveTextEditor();

        bucket.putObject('code/'+editor.getTitle(),
        {'body':
      ...
      this.cloudCodeView.getElement().appendChild(this.cloudCodeView.createButton('Delete',['btn','btn-lg'],() => {
        var fs=require('fs');
      ...
      this.cloudCodeView.getElement().appendChild(this.cloudCodeView.createButton('Config',['btn','btn-lg'],() => {
